...
public class PersonController {
    ...
    public void insert(ModelPerson mod) {
        ...
        try {
            conecta.conexao();
            PreparedStatement pst = conecta.conn.prepareStatement("INSERT INTO person (id, first_name, last_name, office, phone_number, register_date) VALUES (?, ?, ?, ?, ?, ?)");
            pst.setInt(1, mod.getId());
            pst.setString(2, mod.getFirst_name());
            pst.setString(3, mod.getLast_name());
            pst.setString(4, mod.getDepartment());
            pst.setString(5, mod.getPhone());
    ...
    public void update(ModelPerson mod, int id) {
        conecta.conexao();
        try {
            PreparedStatement pst = conecta.conn.prepareStatement("UPDATE person SET first_name= ?, last_name= ?, phone_number= ?, office= ? WHERE id=?");
    ...
    public void fillTable(String SQL, JTable tabela) {
        ...
        String[] Colunas = new String[]{"ID", "", "Name", "Phone", "Function",};
