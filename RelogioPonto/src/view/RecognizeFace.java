...
public class RecognizeFace extends javax.swing.JDialog {
    ...
    CascadeClassifier cascade = new CascadeClassifier("./../RelogioPonto/src/files/haarcascade_frontalface_alt.xml");
    ...
    public RecognizeFace(java.awt.Frame parent, boolean modal) {
        ...
        initComponents();
        try {
            recognizer.read("./../RelogioPonto/src/files/Images/classifierLBPH.yml");
            recognizer.setThreshold(80);
        } catch (Exception e) {
            JOptionPane.showMessageDialog(null, "Não existe usuário cadastrado.");
            dispose();
        }
        startCamera();
    }
    ...
    private void initComponents() {
        ...
        jPanel4.add(saveButton, new org.netbeans.lib.awtextra.AbsoluteConstraints(100, 170, -1, -1));
        saveButton.setCursor(new java.awt.Cursor(java.awt.Cursor.HAND_CURSOR));

        jLabel1.setFont(new java.awt.Font("Segoe UI", 0, 14)); // NOI18N
        jLabel1.setForeground(new java.awt.Color(200, 0, 0));
        jPanel4.add(jLabel1, new org.netbeans.lib.awtextra.AbsoluteConstraints(60, 220, 260, 40));
    ...
    private void saveButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveButtonActionPerformed
        if (idPerson == 0) {
            jLabel1.setText("Usuário não identificado.");
        } else {
            try {
                String date = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss").format(new Date(System.currentTimeMillis()));
                conecta.conexao();
                PreparedStatement pst = conecta.conn.prepareStatement("INSERT INTO checklist (date, idpessoa) VALUES (?, ?)");
                pst.setString(1, date);
                pst.setInt(2, idPerson);
                pst.executeUpdate();
                conecta.desconecta();
                JOptionPane.showMessageDialog(null, "Checked Successfully.");
                dispose();
